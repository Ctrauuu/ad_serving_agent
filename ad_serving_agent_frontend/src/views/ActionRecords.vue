...
<template>
  <main v-loading="loading" class="action-page">
    <header class="page-header">
      <div><el-button link :icon="ArrowLeft" @click="router.push(`/campaigns/${campaignId}/suggestions`)">返回干预建议</el-button><p class="eyebrow">EXECUTION LEDGER</p><h1>动作执行记录 <span v-if="campaign">· {{ campaign.name }}</span></h1><p>记录平台执行结果与可验证的状态快照，安全回滚仅开放给投放负责人。</p></div>
