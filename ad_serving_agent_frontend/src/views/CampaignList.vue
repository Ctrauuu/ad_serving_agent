    ...
    <el-card class="table-card" shadow="never">
      ...
      <el-table v-loading="loading" :data="campaigns" class="campaign-table" row-key="id">
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }"><el-button link type="primary" @click="router.push(`/campaigns/${row.id}/strategy`)">查看</el-button><el-button link type="primary" @click="router.push(`/campaigns/${row.id}/ad-tasks`)">任务</el-button><el-button link type="primary" @click="router.push(`/campaigns/${row.id}/anomalies`)">异常</el-button><el-button link type="primary" @click="router.push(`/campaigns/${row.id}/actions`)">执行</el-button><el-button link type="primary" @click="router.push(`/campaigns/${row.id}/review`)">复盘</el-button></template>
        </el-table-column>
      </el-table>
