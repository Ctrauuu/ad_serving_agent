...
async def parse_goal_text(goal_text:str) -> GoalParseResult:
    try:
        result = await (
            _prompt | get_goal_llm() | _parser
        ).ainvoke({"goal_text": goal_text})
    except OutputParserException as exc:
        raise ValueError("模型返回内容不符合结构化目标格式") from exc

    if result.missing_fields:
        return GoalParseResult(missing_fields=result.missing_fields)

    if result.structured_goal is None:
        raise ValueError("模型未返回结构化目标或缺失字段")
